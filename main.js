export default class UnclickableButton {
  ...
  avoidClick() {
    ...
    this.topPercentPosition = Math.floor(
      Math.random() * maxRandomNumber + minRandomNumber
    );
    this.leftPercentPosition = Math.floor(
      Math.random() * maxRandomNumber + minRandomNumber
    );
    this.clickableElement.setAttribute(
      "style",
      `position:absolute;top:${this.topPercentPosition}%;left:${this.leftPercentPosition}%`
    );
  }
}
